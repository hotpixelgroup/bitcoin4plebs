...
export const quest09: Quest = {
  ...
  stops: [
    {
      ...
      takeaway:
        'Nobody hands your node a balance sheet. It starts from the Quest #8 genesis block and **re-derives the entire state of Bitcoin** — downloading every block since 2009 and re-running every check you\'ve read in this curriculum. Including this old friend, once per block, ~900,000 times.',
      prose: [
        ...
        'Understand what those days of CPU are buying. Your node never asks "what\'s the balance?" — it **recomputes** the answer from raw history, then keeps enforcing the rules on every new block forever. When it finally says "synced," the state it holds isn\'t downloaded trust. It\'s arithmetic your own hardware performed.',
      ],
      annotations: [
    ...
    },
    {
      ...
      takeaway:
        'How does a brand-new node with no account and no permission find the network? **A short list of introduction services, baked into the code, run by named individuals** — used once for directions, then never trusted again.',
      prose: [
        'These are DNS seeds: addresses your node queries once, on first launch, to get an initial list of peers. Look at the comments — they aren\'t companies. They\'re **individual developers, by name**, running a public service. After introductions are made, your node builds and maintains its own address book and never needs them again.',
        'Could a seed lie to you? It could serve a list of hostile peers — and it would gain nothing, because your node **verifies every block those peers send** with the checks from Stop 1. Seeds can make introductions; they cannot vouch for anything. That\'s the design principle of this entire system, applied even to saying hello: verification is never delegated. (The paranoid can skip seeds entirely with `-dnsseed=0` and point at a friend\'s node by hand.)',
      ],
      annotations: [
    ...
    },
    {
      ...
      takeaway:
        'Full disclosure, in this site\'s tradition: out of the box, your node **skips one class of check** for ancient history — signature verification for blocks buried below a reviewed checkpoint. Every other rule still runs on every block. And one flag turns even this shortcut off.',
      prose: [
        'Line 142 names a specific block (height 938,343, released with this version after public review). During initial sync, if that block appears in your chain, blocks buried **beneath** it skip Quest #3\'s expensive signature math — the reasoning being that the developers, and everyone who reviewed the release, already checked those signatures, and a fake ancestry would additionally need more cumulative proof-of-work than the real chain (line 141\'s minimum-work floor).',
        'Know what is **not** skipped — this matters: every block\'s proof-of-work, every subsidy amount, every fee, every double-spend check, the entire supply arithmetic. Those run in full, from genesis, on your machine, regardless. And if borrowing even the signature review offends you — Bitcoin Core\'s own help text says it plainly: start with `-assumevalid=0`, described in the source as *"0 to verify all"*, and your node re-checks every signature since 2009. The shortcut is a default, never a rule. You hold the dial.',
      ],
      annotationsOpen: true,
      annotations: [
        { lines: 'L141', text: 'The work floor: your node ignores any chain claiming less total proof-of-work than this — you can\'t be fed a cheap fake history.' },
        { lines: 'L142', text: 'The reviewed checkpoint. Signatures below it may be skipped; **amounts and supply never are**. Refuse it with `-assumevalid=0`.' },
      ],
      excerpt: {
    ...
    },
    {
      ...
      prose: [
        ...
        'Notice `total_unspendable_amount` two lines down: the coins **permanently excluded** — Quest #8\'s frozen genesis reward among them, alongside rewards miners forgot to claim and outputs provably burned. It\'s why the measured number lands slightly *below* the schedule, and your node itemizes the gap. Even the missing coins are accounted for. (And enjoy line 1041: the developers labeling their own statistic "meaningless." This codebase does not flatter itself.)',
      ],
      annotations: [
        { lines: 'L1040', text: 'How many locked boxes exist right now (tens of millions).' },
  ...
  recap: {
    items: [
      ...
      {
        text: '**`gettxoutsetinfo` measures the money supply from your own disk** — and itemizes every missing satoshi, Satoshi\'s frozen 50 BTC included.',
