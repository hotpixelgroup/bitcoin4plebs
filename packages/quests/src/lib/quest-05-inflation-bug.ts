...
export const quest05: Quest = {
  ...
  stops: [
    {
      ...
      prose: [
        'This little function is where every transaction\'s story begins — coinbase or ordinary, yours or a miner\'s. It doesn\'t know about signatures or balances yet; it asks simpler questions first. Does this transaction have inputs at all? Outputs at all?',
    ...
    },
    {
      ...
      prose: [
        'In 2010, amounts were added up in fixed-size integers with no overflow guard. An attacker crafted two outputs so enormous their *sum* wrapped around — like a car odometer rolling past 999,999 — landing on a small, innocent-looking total. The network accepted it, and for a few hours, 184,467,440,737 BTC existed.',
    ...
    },
    {
      ...
      takeaway:
        'CVE-2018-17144: for nearly two years, a performance optimization had removed the check below. A transaction spending **the same coin twice in one block** could crash nodes — or quietly **inflate the supply**. Here is the restored check, scar named on line 36.',
      prose: [
        'The bug\'s story matters as much as its mechanics. A developer working on Bitcoin Cash spotted the flaw and reported it responsibly. Core developers shipped a fix within hours, initially describing it publicly as a crash bug only — revealing the inflation risk two weeks later, once most of the network had upgraded. No coins were ever printed on Bitcoin.',
        'Read the developers\' comment on lines 37–40: it explains *exactly* what failing to run this check would cost — "either a crash or an inflation bug." This is the honest lesson of the quest: the 21M cap is not a law of physics. It\'s a promise kept by code, **and code is kept honest by people checking it** — which, as of five quests ago, now includes you.',
      ],
      annotations: [
  ...
  finale: {
    ...
    translation: {
      ref: { file: 'this page · faithful translation of tx_check.cpp:23-45', startLine: 1, endLine: 13 },
      language: 'ts',
      lines: [
        ...
        { n: 5, text: '    if (txout.value > MAX_MONEY) return "bad-txns-vout-toolarge";' },
        { n: 6, text: '    nValueOut += txout.value;' },
        { n: 7, text: '    if (!moneyRange(nValueOut)) return "bad-txns-txouttotal-toolarge";' },
        { n: 8, text: '  }' },
        { n: 9, text: '  // Check for duplicate inputs (see CVE-2018-17144)' },
        { n: 10, text: '  const seen = new Set();' },
        { n: 11, text: '  for (const txin of tx.vin) {' },
        { n: 12, text: '    if (seen.has(outpoint(txin))) return "bad-txns-inputs-duplicate";' },
        { n: 13, text: '    seen.add(outpoint(txin)); } return "ok"; }' },
      ],
    },
  ...
  recap: {
    items: [
      ...
      {
        text: '**The 2018 bug could have inflated the supply and was fixed before anyone exploited it** — found by an outside developer reading the code.',
