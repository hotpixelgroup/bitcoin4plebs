...
export const quest13: Quest = {
  ...
  stops: [
    ...
    },
    {
      ...
      prose: [
        "In early 2023, the Ordinals protocol showed how to park entire images inside **taproot witness data**, the signature-carrying part of a transaction. SegWit (Quest #4's activation story) deliberately prices those bytes at a quarter of the cost of normal ones. Suddenly the 80-byte valve was irrelevant; whole JPEGs rode through the signature door. Then BRC-20 tokens turned each inscription into a tradable collectible. Twice in 2023, and again on the 2024 halving day with Runes, data demand outbid payment demand across the whole queue, at hundreds of sats per vbyte.",
    ...
    },
    {
      ...
      prose: [
        ...
        '**The steelman for it:** the auction prices this block, but not the forever-costs some bytes impose on every future node. Collectible manias monetize attention, so they can outbid money indefinitely instead of fading. Policy filters demonstrably leak. And the design is deliberately humble: temporary, narrow, and grandfathered, a circuit breaker to cool a market failure while the ecosystem builds better answers. Bitcoin exists to be money, and to this camp, a rule that defends money-ness is the *most* conservative move available.',
        "**The steelman against it:** Quest #0's founding principle says a rule only counts if checking it needs no referee. \"This data is legitimate, that data is not\" is referee work smuggled into arithmetic. Consensus has never judged *content* before, and one-year emergencies have a habit of renewing. The collateral damage is real too: tapscripts that execute `OP_IF` are the either/or contracts several second-layer designs are built from. And the precedent cuts forever, because the tool that bans their bytes today can ban your bytes when fashions change. The auction being annoying is the price of the auction being neutral.",
      ],
      annotations: [
    ...
    },
    {
      ...
      prose: [
        ...
        'Notice what the machine has already done with this fight, with nobody chairing it. The fee auction priced the flood. The policy layer let dissenters filter in protest. The implementation market gave the protest teeth and a name. Now the last question waits at the consensus layer, wrapped in a deliberately temporary proposal, for the economy to accept or ignore. Thirteen quests ago you had to trust someone\'s summary of a fight like this. Today you can read the filter, the default, and the proposal in their own words, and decide what *you* will run.',
      ],
    },
  ],
  finale: {
  ...
  recap: {
    items: [
      ...
      {
        text: '**BIP-110 would escalate taste into law**: one year, grandfathered, narrow, and consensus-binding. That is exactly why it reads as a circuit breaker to one camp and a precedent to the other.',
  ...
  feynman: {
    ...
    model:
      'Everyone agrees the data is consensus-valid and paid for. The fight is about the auction\'s neutrality: collectibles can outbid payments, and some bytes stick every future node with storage costs, so is that the system working, or an incentive worth correcting? One camp says filters leak, and only a temporary, grandfathered consensus rule can defend the ledger\'s purpose as money. The other says consensus judging content breaks the no-referee principle that makes Bitcoin worth defending at all. Both camps want Bitcoin to survive; they disagree about which wall is load-bearing.',
  },
};
