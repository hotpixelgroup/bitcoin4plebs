 ...
 * per block, one block every ten minutes.
 */
      ...
      <div>
        <div className="viz-root">
          ...
          <div className="viz-sub">
            vbytes waiting at each feerate · ▲ marks your bid · top bands confirm first
          </div>
          <div className="band-chart">
            {[...bands].reverse().map((band) => {
              const mine = feerate >= band.from && feerate < band.to;
              return (
                <div key={band.from} className="band-row">
                  ...
                  <span className="band-bar-track">
                    <span
                      ...
                      style={{ width: `${Math.max(2, (band.vbytes / maxBand) * 100)}%` }}
                    />
                  </span>
                  <span className="band-value">
                    {(band.vbytes / 1_000_000).toFixed(2)} MvB
